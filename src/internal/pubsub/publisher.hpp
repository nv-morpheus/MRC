...
template <typename T>
class Publisher : public srf::node::SourceChannelWriteable<T>
{
    Publisher(std::string service_name,
              std::uint64_t tag,
              control_plane::client::SubscriptionService& subscription_service) :
  ...
  private:
    ...
    control_plane::client::SubscriptionService& m_subscription_service;
...
class PublisherManager : public PublisherManagerBase
{
  ...
  private:
    ...
    void do_service_start() override
    {
        ...
        auto publisher = std::shared_ptr<Publisher<T>>(new Publisher<T>(service_name(), this->tag(), *this),
                                                       [this](Publisher<T>* ptr) {
