...
class Client final : public resources::PartitionResourceBase, public Service
{
  ...
  private:
    ...
    // update channel
    std::unique_ptr<srf::node::WritableSubject<const protos::StateUpdate>> m_connections_update_channel;
    std::map<InstanceID, srf::node::WritableSubject<const protos::StateUpdate>> m_instance_update_channels;
