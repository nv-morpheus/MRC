...
function str2array(str: string | readonly string[] | undefined): readonly string[] | undefined {
...
class DevToolsEnhancer<S, A extends Action<unknown>> {
            ...
            type === "ERROR"
               ...
               : stringify(
                    filterState(
                       ...
                       type,
                       this.filters,
                       this.stateSanitizer as (state: unknown, index?: number) => unknown,
                       this.actionSanitizer as ((action: Action<unknown>, id: number) => Action) | undefined,
                       nextActionId!
                    )
                 );
      }
      if (type === "ACTION") {
         message.action = stringify(
            !this.actionSanitizer
               ? action
               : this.actionSanitizer((action as PerformAction<A>).action, nextActionId! - 1)
         );
   ...
   dispatchRemotely(action: string | { args: string[]; rest: string; selected: number }) {
      try {
         const result = evalAction(action, this.actionCreators as ActionCreatorObject[]);
   ...
   init(options: Options<S, A>) {
      ...
      if (options.port) {
         ...
         this.socketOptions = defaultSocketOptions;
      }
      ...
      if (this.sendOn || this.sendOnError) {
         this.sendTo =
            options.sendTo ||
            `${this.socketOptions.secure ? "https" : "http"}://${this.socketOptions.hostname}:${
   ...
   stop = (keepConnected?: boolean) => {
      ...
      if (!keepConnected) {
         ...
         this.socket.closeAllListeners();

         delete this.socket;
      }

      // if (this.subscribeListener) {
      //    this.subscribeListener.kill();
      //    delete this.subscribeListener;
      // }

      // if (this.connectListener) {
      //    this.connectListener.kill();
      //    delete this.connectListener;
      // }

      // if (this.disconnectListener) {
      //    this.disconnectListener.kill();
      //    delete this.disconnectListener;
      // }

      // if (this.errorListener) {
      //    this.errorListener.kill();
   ...
   monitorReducer = (state = {}, action: LiftedAction<S, A, {}>) => {
      ...
      if (!this.started && this.sendOnError === 2 && this.store.liftedStore) {
         ...
         if (
            this.startOn &&
            !this.started &&
            this.startOn.indexOf((action as PerformAction<A>).action.type as string) !== -1
         ) {
            async(this.start);
         } else if (
            this.stopOn &&
            this.started &&
            this.stopOn.indexOf((action as PerformAction<A>).action.type as string) !== -1
         ) {
            async(this.stop);
         } else if (
            this.sendOn &&
            !this.started &&
            this.sendOn.indexOf((action as PerformAction<A>).action.type as string) !== -1
         ) {
            async(this.send);
         }
      }
   ...
   enhance = (options: Options<S, A> = {}): StoreEnhancer => {
      ...
      const realtime =
         typeof options.realtime === "undefined" ? process.env.NODE_ENV === "development" : options.realtime;
...
export default <S, A extends Action<unknown>>(options?: Options<S, A>) => new DevToolsEnhancer<S, A>().enhance(options);
...
const compose =
   ...
   (...args: unknown[]) => {
      ...
      function preEnhancer(createStore: StoreEnhancerStoreCreator) {
         return <S, A extends Action<unknown>>(reducer: Reducer<S, A>, preloadedState: PreloadedState<S>) => {
      ...
      return [preEnhancer, ...funcs].reduceRight(
         ...
         devToolsEnhancer.enhance(options)(...(args as [StoreEnhancerStoreCreator]))
      );
   };

export function composeWithDevTools(...funcs: [Options<unknown, Action<unknown>>] | StoreEnhancer[]) {
   if (funcs.length === 0) {
