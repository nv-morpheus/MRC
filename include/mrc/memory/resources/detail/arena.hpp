...
class block
{
  public:
    ...
    [[nodiscard]] bool is_superblock() const
    {
        return size_ >= minimum_superblock_size;
    }
     ...
     */
    [[nodiscard]] bool fits(std::size_t size) const
    {
        return size_ >= size;
    }
    ...
    bool operator<(block const& blk) const
    {
        return pointer_ < blk.pointer_;
    }

  private:
    char* pointer_{};     ///< Raw memory pointer. // NOLINT
    std::size_t size_{};  ///< Size in bytes. // NOLINT
};

inline bool block_size_compare(block lhs, block rhs)
{
...
class global_arena final
{
  public:
    ...
    static constexpr std::size_t default_initial_size = std::numeric_limits<std::size_t>::max();  // NOLINT
    /// The default maximum size for the global arena.
    static constexpr std::size_t default_maximum_size = std::numeric_limits<std::size_t>::max();  // NOLINT
    /// Reserved memory that should not be allocated (64 MiB).
    static constexpr std::size_t reserved_size = 1U << 26U;  // NOLINT
...
class arena
{
  public:
     ...
     */
    explicit arena(std::shared_ptr<global_arena<Upstream>> global_arena) : global_arena_{std::move(global_arena)} {}
  ...
  private:
    ...
    static constexpr int max_free_blocks = 16;  // NOLINT
     ...
     */
    void shrink_arena(block const& blk)
    {
    ...
    std::shared_ptr<global_arena<Upstream>> global_arena_;  // NOLINT
    /// Free blocks.
    std::set<block> free_blocks_;  // NOLINT
    /// Mutex for exclusive lock.
    mutable std::mutex mtx_;  // NOLINT
