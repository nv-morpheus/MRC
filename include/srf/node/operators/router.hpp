...
template <typename KeyT, typename T>
class Router : public RouterBase<KeyT, std::pair<KeyT, T>, T>
{
  protected:
    ...
    virtual KeyT determine_key_for_value(const input_data_t& t)
    {
        return t.first;
    }

    output_data_t convert_value(input_data_t&& t) override
    {
        return t.second;
    }
};
