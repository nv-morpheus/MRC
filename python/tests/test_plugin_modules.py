...
def test_plugin_module_create_or_acquire():
    mod = mrc.PluginModule.create_or_acquire("doesnt_exist.so")
...
def test_dynamic_module_plugin_interface():
    plugin_module = mrc.PluginModule.create_or_acquire("libdynamic_test_module.so")
...
def test_dynamic_module_registration():
    ...
    module_namespace = "mrc_unittest_cpp_dynamic"
...
def test_dynamic_module_plugin_registration():
    ...
    assert "mrc_unittest_cpp_dynamic" in registered_modules

    ns_1 = registered_modules["mrc_unittest_cpp_dynamic"]
    assert len(ns_1) == 1
    assert ns_1[0] == "DynamicSourceModule"

    assert "mrc_unittest_cpp_dynamic_2" in registered_modules

    ns_2 = registered_modules["mrc_unittest_cpp_dynamic_2"]
    assert len(ns_2) == 1
    assert ns_2[0] == "DynamicSourceModule"

    assert "mrc_unittest_cpp_dynamic_3" in registered_modules

    ns_3 = registered_modules["mrc_unittest_cpp_dynamic_3"]
    assert len(ns_3) == 1
    ...
    assert "mrc_unittest_cpp_dynamic" not in registered_modules
    assert "mrc_unittest_cpp_dynamic_2" not in registered_modules
    assert "mrc_unittest_cpp_dynamic_3" not in registered_modules
...
def test_dynamic_module_bad_version_test():
    ...
    module_namespace = "mrc_unittest_cpp_dynamic_BAD"
