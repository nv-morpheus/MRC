...
struct EdgeAdapterUtil
{
    using source_adapter_fn_t = std::function<std::shared_ptr<node::IEdgeWritableBase>(
        srf::node::SourcePropertiesBase&, srf::node::SinkPropertiesBase&, std::shared_ptr<node::IEdgeWritableBase>)>;
    using sink_adapter_fn_t   = std::function<std::shared_ptr<node::IEdgeWritableBase>(
        std::type_index, srf::node::SinkPropertiesBase&, std::shared_ptr<node::IEdgeWritableBase> ingress_handle)>;
    ...
    static void register_data_adapters()
    {
        ...
        //     VLOG(2) << "Registering PySRF sink adapter for: " << type_name<DataTypeT>() << " " <<
