...
class LifeCycleStreaming : public IContextLifeCycle
{
  ...
  protected:
    ...
    virtual void StreamFinished(std::shared_ptr<ServerStream>) {}
...
template <class Request, class Response>
bool LifeCycleStreaming<Request, Response>::RunNextState(bool (LifeCycleStreaming<Request, Response>::*state_fn)(bool),
                                                         bool ok)
{
...
typename LifeCycleStreaming<Request, Response>::Actions LifeCycleStreaming<Request, Response>::EvaluateState()
{
    ...
    {
        DVLOG(100) << "Stream was CANCELLED by Server - Cancel All Callbacks";
    ...
    // clang-format off
        DVLOG(100) << (should_read ? 1 : 0) << (should_write ? 1 : 0) << (should_execute ? 1 : 0)
               << (should_finish ? 1 : 0)
               << " -- " << m_Reading << m_Writing
...
void LifeCycleStreaming<Request, Response>::ForwardProgress(Actions& actions)
{
    ...
    if (should_read)
    {
        DVLOG(100) << "Posting Read/Recv";
        m_Stream->Read(&m_RequestQueue.back(), m_ReadStateContext.IContext::Tag());
    }
    if (should_finish)
    {
        DVLOG(100) << "Closing Stream - " << (m_Status->ok() ? "OK" : "CANCELLED");
...
bool LifeCycleStreaming<Request, Response>::StateInitializedDone(bool ok)
{
    ...
    OnLifeCycleStart();
    {
        ...
        m_ServerStream   = std::shared_ptr<ServerStream>(new ServerStream(this), [this](auto ptr) mutable {
            ...
            {
                std::lock_guard<std::recursive_mutex> lock(m_QueueMutex);
                DVLOG(100) << "All ServerStream objects have been deleted";
...
bool LifeCycleStreaming<Request, Response>::StateReadDone(bool ok)
{
    ...
    {
        std::lock_guard<std::recursive_mutex> lock(m_QueueMutex);
        DVLOG(100) << "ReadDone Event: " << (ok ? "OK" : "NOT OK");
        ...
        {
            {
                ...
                DVLOG(100) << "WritesDone received from Client; closing Server Reads";
                m_ReadsDone = true;
                m_RequestQueue.pop();
                m_ReadStateContext.m_NextState = &LifeCycleStreaming<RequestType, ResponseType>::StateInvalid;
            }
...
bool LifeCycleStreaming<Request, Response>::StateWriteDone(bool ok)
{
    ...
    {
        std::lock_guard<std::recursive_mutex> lock(m_QueueMutex);
        DVLOG(100) << "WriteDone Event: " << (ok ? "OK" : "NOT OK");

        m_Writing = false;

        if (!ok)
        {
            DVLOG(100) << "Write Response failed on Stream; Client may have Cancelled";
            CancelResponse();
        }
        else
        {
...
{
    DVLOG(100) << "Server closed Write stream - FinishedDone - " << (ok ? "OK" : "CANCELLED");
...
void LifeCycleStreaming<Request, Response>::BlockFinish()
{
    {
        std::lock_guard<std::recursive_mutex> lock(m_QueueMutex);
        DVLOG(100) << "Setting BlockFinish to true; UnblockFinish must be called for RPC to complete";
        m_BlockFinish = true;
    }
}
...
void LifeCycleStreaming<Request, Response>::UnblockFinish()
{
    ...
    {
        std::lock_guard<std::recursive_mutex> lock(m_QueueMutex);
        DVLOG(100) << "Setting BlockFinish to false - this will also trigger a step thru the progress engine";
        m_BlockFinish = false;
    }
    ForwardProgress(actions);
}
...
void LifeCycleStreaming<Request, Response>::CloseStream(::grpc::Status status)
{
    ...
    {
        std::lock_guard<std::recursive_mutex> lock(m_QueueMutex);
        DVLOG(100) << "Queue Close Stream: " << (status.ok() ? "OK" : "NOT OK");
        ...
        if (!m_ReadsDone)
        {
            DVLOG(100) << "Server Canceling before Client WritesDone; issue TryCancel() to flush Read Tags";
