...
struct ClientStreaming : public BaseContext
{
  ...
  public:
    ClientStreaming();
    ~ClientStreaming() override = default;
  ...
  protected:
    virtual void TimeoutBackoff(const std::uint64_t&) {}

    virtual std::unique_ptr<::grpc::ClientAsyncReaderWriter<Request, Response>> PrepareAsync(
        ::grpc::ClientContext*, ::grpc::CompletionQueue*) = 0;
  ...
  private:
    ...
    class Context : public BaseContext
    {
      ...
      private:
        bool RunNextState(bool ok) final override
        {
            // DVLOG(100) << this << ": " << "Event for Tag: " << Tag();
...
template <typename Request, typename Response>
ClientStreaming<Request, Response>::ClientStreaming() :
...
void ClientStreaming<Request, Response>::Initialize()
{
    ...
    m_Stream    = PrepareAsync(&m_Context, m_CQ.get());
...
bool ClientStreaming<Request, Response>::Write(Request&& request)
{
    Actions actions;
    {
        ...
        DVLOG(100) << this << ": Queuing Write Request";
        DVLOG(100) << this << ": On Queuing Write, there are " << m_WriteQueue.size() << " outstanding writes";
...
bool ClientStreaming<Request, Response>::CloseWrites()
{
    Actions actions;
    {
        ...
        DVLOG(100) << this << ": Queuing WritesDone - no more Writes can be queued";
        DVLOG(100) << this << ": On Queuing WritesDone, there are " << m_WriteQueue.size() << " outstanding writes";
...
void ClientStreaming<Request, Response>::Cancel()
{
    ...
    {
        std::lock_guard<std::mutex> lock(m_Mutex);
        DVLOG(100) << this << ": Client Cancelling Stream";
        DVLOG(100) << this << ": On Cancel, there are " << m_WriteQueue.size() << " outstanding writes";
...
typename ClientStreaming<Request, Response>::Actions ClientStreaming<Request, Response>::EvaluateState()
{
    ...
    {
        DVLOG(100) << this << ": Action Queued: Stream Initializing";
    }
    else
    {
        ...
        if (!m_Writing && !m_WriteQueue.empty())
        {
            ...
            DVLOG(100) << this << ": WriteQueue has " << m_WriteQueue.size() << " outstanding messages";
        }
    ...
    // clang-format off
                DVLOG(100) << this << ": " << (should_read ? 1 : 0) << (should_write ? 1 : 0) << (should_finish ? 1 : 0)
...
void ClientStreaming<Request, Response>::ForwardProgress(Actions& actions)
{
    ...
    if (should_read)
    {
        DVLOG(100) << this << ": Posting Read/Recv";
        m_Stream->Read(&m_ReadQueue.back(), m_ReadState.Tag());
    }
    if (should_write)
    {
        DVLOG(100) << this << ": Writing/Sending Request";
        if (m_Corked)
        {
    ...
    if (should_close)
    {
        DVLOG(100) << this << ": Sending WritesDone to Server";
        m_Stream->WritesDone(m_WriteState.Tag());
    }
    if (should_finish)
    {
        DVLOG(100) << this << ": Closing Stream - Finish";
...
bool ClientStreaming<Request, Response>::StateStreamInitialized(bool ok)
{
    ...
    {
        std::lock_guard<std::mutex> lock(m_Mutex);
        DVLOG(100) << this << ": StreamInitialized";
...
bool ClientStreaming<Request, Response>::StateReadDone(bool ok)
{
    ...
    {
        std::lock_guard<std::mutex> lock(m_Mutex);
        DVLOG(100) << this << ": ReadDone: " << (ok ? "OK" : "NOT OK");
        ...
        if (!ok)
        {
            DVLOG(100) << this << ": Server is closing the read/download portion of the stream";
            m_ReadsDone  = true;
            m_WritesDone = true;
            if (m_Writing)
            {
...
bool ClientStreaming<Request, Response>::StateWriteDone(bool ok)
{
    ...
    {
        std::lock_guard<std::mutex> lock(m_Mutex);
        DVLOG(100) << this << ": WriteDone: " << (ok ? "OK" : "NOT OK");
...
bool ClientStreaming<Request, Response>::StateWritesDoneDone(bool ok)
{
    ...
    {
        std::lock_guard<std::mutex> lock(m_Mutex);
        DVLOG(100) << this << ": WritesDoneDone: " << (ok ? "OK" : "NOT OK");
...
bool ClientStreaming<Request, Response>::StateFinishDone(bool ok)
{
    ...
    {
        std::lock_guard<std::mutex> lock(m_Mutex);
        DVLOG(100) << this << ": FinishedDone: " << (ok ? "OK" : "NOT OK");
...
template <typename Request, typename Response>
void ClientStreaming<Request, Response>::ProgressEngine()
{
    CHECK(!m_IsRunning);
    m_IsRunning = true;
    ...
    while (true)
    {
        switch (m_CQ->AsyncNext<gpr_timespec>(&tag, &ok, gpr_time_0(GPR_CLOCK_REALTIME)))
        {
        case grpc::CompletionQueue::NextStatus::GOT_EVENT: {
            BaseContext* ctx = BaseContext::Detag(tag);
            // DVLOG(3) << "streaming client progress engine issuing callback";
            if (!ctx->RunNextState(ok))
            {
                // we are done here
                DVLOG(100) << "stream client progress engine finished";
                return;
            }
            // DVLOG(3) << "streaming client progress engine callback complete";
            backoff = 1;
        }
        break;
        ...
        break;
        default:
            LOG(FATAL) << "unknown CQ::NextStatus";
        }
    }
}

template <typename Request, typename Response>
void ClientStreaming<Request, Response>::Shutdown()
{
    m_CQ->Shutdown();
}
