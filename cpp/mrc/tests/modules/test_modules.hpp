...
namespace mrc {
    ...
    public:
        MultiSourceModule(std::string module_name)
    ...
    public:
        MultiSinkModule(std::string module_name)
                : SegmentModule(std::move(module_name)) {}
    ...
    void MultiSinkModule<DataTypeT, SinkCountV>::initialize(segment::Builder &builder) {
        for (std::size_t i = 0; i < SinkCountV; ++i) {
            auto input = builder.make_sink<DataTypeT>("input_" + std::to_string(i),
                                                      [this, i](DataTypeT data) {
                                                          m_received_count[i]++;
                                                      });
