...
class Engines : public IEngines
{
  public:
    Engines(LaunchOptions launch_options);
    ~Engines() override;

    const std::vector<std::shared_ptr<IEngine>>& launchers() const final;
    const LaunchOptions& launch_options() const final;
    std::size_t size() const final;

  protected:
    void add_launcher(std::shared_ptr<IEngine> launcher);
    void clear_launchers();

  private:
    LaunchOptions m_launch_options;
    std::vector<std::shared_ptr<IEngine>> m_launchers;
};
